...
export default class Tile3D {
    ...
    rotate(num) {
        ...
        for (let i = 0; i < 2; i++) {
            // -1이면 돌릴 필요 없음
            if (this.faces[i] !== '-1') {
                newFaces[i] = this.faces[i].slice(0, -1) +
                    String.fromCharCode((this.faces[i].slice(-1).charCodeAt(0) - 'a'.charCodeAt(0) + num) % 4 + 'a'.charCodeAt(0));
            }
            else{
                newFaces[i] = '-1';
            }
    ...
    analyze(tiles3D) {
        for (let i = 0; i < tiles3D.length; i++) {
            ...
            // BRFL Setting
            for (let j = 2; j < 6; j++) {
                if (compareFaceBRFL(tile3D.faces[(j + 4) % 4 + 2], this.faces[j])) {
                    this.constraint[j].push(i);
                }
            }
        }
