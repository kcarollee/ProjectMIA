...
export default class WFC3D {
    ...
    constructor(modelCount, rulebook, urlString, formatString) {
        // 타일 로드
        this.modelCount = modelCount;
        this.meshes = [];
        this.promises = [];
        this.tiles3D = [];
        this.loadGLTFs(urlString, formatString);
        this._WFC3DInit(modelCount, rulebook);
    }

    loadGLTFs(urlString, formatString) {
        const loader = new GLTFLoader();
        for (let i = 0; i < this.modelCount; i++) {
            this.promises[i] = new Promise(resolve => {
                loader.load(urlString + i + formatString, resolve, undefined, function (error) {
                    console.log(error);
                })
            })
        }
    ...
    _WFC3DInit(modelCount, rulebook) {
        Promise.all(this.promises).then(gltf => {

            // Add Empty model
            this.tiles3D.push(new Tile3D("EMPTY MESH", ['-1', '-1', '-1', '-1', '-1', '-1',], 0));

            // load Model to Tiles3D
            for (let i = 0; i < modelCount; i++) {
                this.tiles3D[i + 1] = new Tile3D(gltf[i].scene.children[0].clone(), rulebook[i], 0);
            }
            // Rotate All model
            for (let i = 1; i < 4; i++) {
        ...
        for (let k = 0; k < dim[2] + 2; k++) {
            for (let j = 0; j < dim[1] + 2; j++) {
                for (let i = 0; i < dim[0] + 2; i++) {
                    // 맨 바깥은 빈 공간 삽입
                    if (k === 0 || k === dim[2] + 1 || j === 0 || j === dim[1] + 1 || i === 0 || i === dim[0] + 1) {
            ...
            for (let j = 1; j < xyGrid.length - 1; j++) {
                ...
                for (let i = 1; i < xGrid.length - 1; i++) {
                    ...
                    for (let dir = 0; dir < 6; dir++) {
                        let pos = [curPos[0] + WFC3D.dx[dir], curPos[1] + WFC3D.dy[dir], curPos[2] + WFC3D.dz[dir]]
    ...
    createBuilding(dim, size) {
        ...
        while (wfc3dIterCnt--) {
            [grid, selectedArr[wfc3dIterCnt]] = this.waveFunctionCollapse3DSingleIteration(grid);
