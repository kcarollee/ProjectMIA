...
export default class WFCFloorMesh {
  ...
  constructor(
    ...
    // load road textures
    for (let i = 0; i < 54; i++) {
      let tex = new THREE.TextureLoader().load(urlString + i + formatString);
  ...
  calcBuildingTransform() {
    ...
    for (let j = 0; j < this.DIM; j++) {
      for (let i = 0; i < this.DIM; i++) {
        ...
        if (
          this.tiles[this.grid[index].options[0]].buildingSpace[1] === this.r0
        ) {
          ...
          do {
            ...
            gridCnt[0]++;
            buildingSpace[0] +=
              this.tiles[this.grid[insideIndex].options[0]].buildingSpace[1] +
        ...
        if (
          this.tiles[this.grid[index].options[0]].buildingSpace[2] === this.u0
        ) {
          ...
          do {
            ...
            gridCnt[1]++;
            buildingSpace[1] +=
              this.tiles[this.grid[insideIndex].options[0]].buildingSpace[0] +
